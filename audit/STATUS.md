...
Last updated 13.09.2026. Game version 1.3.20. Pass 1 is paused after wave 2.

## Where things stand

- Pass 1 has 12 of 70 batches done: B01 to B09 and the catch-up batches R01 to R03 are scanned, triaged, applied, validated and committed.
- 58 batches are left, B10 to B67. At 6 batches per wave that is 10 more waves (wave 3 to wave 12).
- The next wave is wave 3: B10, B11, B12, B13, B14, B15, all Diplomacy.xml.
...
- The address to the player is only partly converted. It is «ви» in the Actions.xml flavor texts and in B07 to B09, but Diplomacy, Quests, Factions and the later flavor files still say «ти» until their waves run. Re-publishing before the audit ends ships that mix.

## Wave plan for the rest of pass 1

- Wave 3: B10 to B15 (Diplomacy.xml)
...
## What was applied (480 fix operations)
...
- Keep as they are: the capitalised «НЕ-» prefix, the EmulatedMind wordplay «заDOOMив» and «DOOM.ати», and names written out in words where Ukrainian needs a case that a `<string/>` reference can't give.
...
- `pass1/batches/`: batch text files. B10 to B67 are waiting to be scanned.
- `pass1/findings/`: one JSON per scanned batch, plus SWEEP, BLEED, BLEED2 and PSCITIES.
- `pass1/scanned_ids.txt`: every entry id already scanned (B01 to B09). `pass1/catchup_ids.txt`: the ids R01 to R03 covered.
- `pass1/wave1.json`, `pass1/wave1.md`, `pass1/wave2.json`, `pass1/wave2.md`: exactly what each wave applied.
...
2. **Check the batches are current.** Batch files are snapshots of the Ukrainian text. They stay valid as long as nothing touched `ModData/Data/Core/Languages/Ukrainian` since the last audit commit. If something did (a game patch sync, a manual fix), rebuild the remaining batches first:
    ...
    ```
    python audit/scripts/build_batches.py pass1 --start 10 --exclude audit/pass1/scanned_ids.txt
    ```

    This rewrites B10 onward and their ledger lines. B01 to B09 stay as they are. New entries from a patch in already-scanned files land in the rebuilt batches, because their ids aren't in `scanned_ids.txt`.

3. **Launch the wave.** One background agent per batch, type `general-purpose`, model `opus`, all six in one message. The prompt for B10 (swap the id for the others):

    ```
    Scan batch B10 of the ZEPHON Ukrainian translation audit. Do all three jobs from the brief.

    1. Read <repo>\audit\BRIEF.md and follow it exactly.
    2. Read the batch: <repo>\audit\pass1\batches\B10.txt
    3. Write your findings to <repo>\audit\pass1\findings\B10.json
...
4. **Post progress every 30 seconds.** Start a persistent Monitor with this loop and relay each line to VK:
    ...
    while true; do
      sleep 30
      finished=""; left=""
      for b in B10 B11 B12 B13 B14 B15; do
        if [ -f "$D/$b.json" ]; then finished="$finished $b"; else left="$left $b"; fi
      done
      mins=$(( ($(date +%s) - start) / 60 ))
      if [ -z "$left" ]; then echo "wave 3: all 6 batches done after ${mins} min"; exit 0; fi
      echo "wave 3: done:${finished:- none}, still running:${left}, ${mins} min"
    done
    ```
...
5. **If an agent fails** (for example "Can't reach the API server"), resume it with SendMessage to its agent id rather than starting over:
    ...
    ```
    Your run stopped on a network error before you wrote the findings file; please continue batch B10 now. Finish the task exactly as your original instructions and BRIEF.md describe, write the findings file, and reply with the one-line summary.
    ```

6. **Triage each batch as it lands.**

    ```
    python audit/scripts/show_findings.py pass1 B10
    python audit/scripts/show_diff.py pass1 B10
    ```
...
7. **Checkpoint with VK.** Merge and dry-run:
    ...
    ```
    python audit/scripts/merge_approved.py pass1 wave3 B10 B11 B12 B13 B14 B15
    python audit/scripts/apply_fixes.py audit/pass1/wave3.json --dry-run
    ```

    Every fix must report as applicable. Summarize the counts and the worst findings, point VK at `pass1/wave3.md`, and ask before applying.

8. **Apply and validate.**

    ```
    python audit/scripts/apply_fixes.py audit/pass1/wave3.json
    ...
    ```
    python -c "import re; ids=[i for b in ['B10','B11','B12','B13','B14','B15'] for i in re.findall(r'^## (.+)$', open(f'audit/pass1/batches/{b}.txt', encoding='utf-8').read(), re.M)]; open('audit/pass1/scanned_ids.txt', 'a', encoding='utf-8', newline='\n').write('\n'.join(ids) + '\n')"
    ```
...
- The validator's NOTES list (seven `<string/>` gaps, Branches.xml as a mod-only file, the empty Labels.xml) and the «заDOOMив» allowance are expected, not defects.

## Cost and timing

- A wave of 6 batches takes 40 to 60 minutes and roughly 700,000 agent tokens (100,000 to 140,000 per batch).
- Diplomacy and Quests batches will likely take longer, because most of their entries need «ви» rewrites.
